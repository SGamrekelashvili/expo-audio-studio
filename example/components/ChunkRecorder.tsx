...
import ExpoAudioStudio, { AudioChunkEvent } from 'expo-audio-studio'
...
const bytesToBase64 = (bytes: Uint8Array) => {
  let binary = ''
  const chunkSize = 0x8000 // to avoid call stack issues
 ...
 const chunksToWav = async (byteChunks: Uint8Array[], filename: string) => {
  if (byteChunks.length === 0) {
    throw new Error('No audio chunks to process')
  }

  console.log(`Processing ${byteChunks.length} chunks...`)
  ...
  const wavFileBase64 = bytesToBase64(completeWavData)
...
const ChunkRecorder = () => {
  const chunkRef = useRef<Uint8Array[]>([])
    ...
    ExpoAudioStudio.addListener('onAudioChunk', (chunk: AudioChunkEvent) => {
      ...
      if (chunk?.type === 'batch' && chunk?.format === 'uint8array' && Array.isArray(chunk?.chunks)) {
        ...
        for (const item of chunk.chunks) {
          if (item?.hasVoice && Array.isArray(item.data)) {
  ...
  return (
    <View>
      {isRecording ? (
        <Text onPress={handleStopRecording} style={{ fontSize: 20, fontWeight: 'bold' }}>
          Stop Recording
        </Text>
      ) : (
        <Text onPress={handleStartRecording} style={{ fontSize: 20, fontWeight: 'bold' }}>
          ChunkRecorder
        </Text>
      )}
    </View>
